...
def add_labels(x, y, axes):
    for i in range(len(x)):
        axes[0].text(x[i], y[i], np.round(y[i],1), fontsize='x-small', horizontalalignment='center', color='purple')
...
def plot_amp_gain(x, y, fig, figure_canvas_agg):
    color = "#c0ffb3"
    tilt = y[-1] - y[0]
    axes = fig.axes
    axes[0].cla()
    axes[0].bar(x, y, width=5)
    axes[0].set_title('Amplifier Gain vs. Frequency', color=color)
    axes[0].set_xlabel('Frequency (MHz)')
